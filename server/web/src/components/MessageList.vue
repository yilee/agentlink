...
useFilePathClick(wrapperRef, filesStore.filePreview.openPreview);
...
// ── Scroll tracking ──
// VList emits onScroll with the scroll offset (number).
// We need to detect if user scrolled up to suppress auto-scroll-to-bottom.
function onVListScroll(offset) {
  ...
  // Emit a synthetic scroll event with enough info for the store
  emit('scroll', { _virtuaOffset: offset, _distanceFromBottom: distanceFromBottom });
}
...
function updateFollowing(offset) {
  if (!vlistRef.value) return;
  const scrollSize = vlistRef.value.scrollSize;
  const viewportSize = vlistRef.value.viewportSize;
  _isFollowing = (scrollSize - offset - viewportSize) <= 80;
}

function onScrollInternal(offset) {
  updateFollowing(offset);
  onVListScroll(offset);
}
...
// Also watch streaming message content changes
watch(() => {
  const last = props.messages[props.messages.length - 1];
  return last && last.isStreaming ? last.content?.length : 0;
}, () => {
  nextTick(() => scrollToBottom(false));
});

// Expose scrollToBottom so store can call it
defineExpose({ vlistRef, scrollToBottom });
...
<template>
  <div ref="wrapperRef" :class="['message-list-wrapper', { compact }]">
