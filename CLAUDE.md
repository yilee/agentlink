...
├── server/web-src/      # Vue 3 SPA source (Vite + Vue SFC), builds to server/web/
│   ├── src/
...
├── server/web/          # Build output (gitignored) — generated by `npm run build` in web-src/
...
**Vue 3 SFC app** (`server/web-src/src/`): Built with Vite + Vue 3 Single-File Components.
...
**Build:** `vite build` → `server/web/` (gitignored). Dependencies: vue ^3.5, marked, highlight.js, pako, tweetnacl.
